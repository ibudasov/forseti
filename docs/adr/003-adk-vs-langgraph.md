...
The roadmap calls for turning the linear pipeline (structured data → rules engine → RAG evidence → recommendation) into a traceable multi-step agentic workflow, with a supervisor agent orchestrating specialist agents (Fundamental Analyst, Technical Analyst, Decision Synthesizer, Critic/Guardrail) and deterministic tool-agents (Input Resolver, Structured Data Collector, Retriever, Risk Manager).
...
3. **Built-in tracing/evaluation hooks** — ADK agent runs emit structured events per step, which map onto the OpenTelemetry spans and `agent_runs`/`agent_run_steps` persistence planned for future observability work, without a bespoke tracing shim.
