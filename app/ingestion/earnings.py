...
def validate_earnings_payload(payload_text: str) -> str:
    ...
    if refusal_reason is not None:
        raise EarningsSourceError(
            f"{refusal_reason}. EARNINGS_CALENDAR is a premium endpoint that a free-tier key cannot read."
        )

    first_line = _first_meaningful_line(payload_text)
    if _REQUIRED_CSV_COLUMN not in first_line.split(","):
        raise EarningsSourceError(f"Invalid earnings calendar payload: {first_line[:120]!r}")
...
def _fallback_earnings_events(active_tickers: dict[str, int]) -> list[EarningsEvent]:
    try:
        import yfinance as yf
    except ImportError:
        logger.exception("earnings_fallback_unavailable")
        return []

    events: list[EarningsEvent] = []
    for ticker, security_id in active_tickers.items():
        try:
            ...
            logger.exception("earnings_fallback_failed: ticker=%s", ticker)
    return events
...
def ingest_earnings(engine=None, ticker: Optional[str] = None) -> tuple[int, list[str]]:
    ...
    if not events:
        logger.error("earnings_ingestion_failed: reason=no_rows_matched active_tickers=%s", len(active_tickers))
        return 0, [SOURCE_FAILURE_MARKER]
